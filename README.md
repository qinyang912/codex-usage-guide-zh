...
> 更新日期：2026-07-22。本指南面向使用自己 ChatGPT 账号的用户，帮助区分 ChatGPT Plus、ChatGPT Pro、Codex 用量、ChatGPT Credits 与 OpenAI API 账单。
...
- [Codex Plugins 创建、安装与 Marketplace](guides/codex-plugins-create-install-marketplace.md)：配置 `plugin.json`、repo/personal Marketplace、`/plugins`，并排查插件不显示。
- [Codex Chrome 插件、Browser 与 Computer Use](guides/codex-chrome-extension-browser-computer-use.md)：选择正确浏览器控制入口，覆盖安装、未连接、商店无法下载和 plugins unavailable。
...
Credits 是套餐包含用量耗尽后的按需余额，不会取代套餐原有的包含用量。对符合条件的账号，Codex、ChatGPT Work、ChatGPT for Excel 等受支持功能可以共享 agentic usage 或 Credits；功能是否可用及实际扣减范围应以账号内 Usage 页面显示为准。

符合条件的 Plus / Pro 用户可在 ChatGPT 网页版或 Codex 应用进入 `Codex Settings → Usage / Usage Dashboard → Credits` 查看余额和购买入口。账号支持时还可以开启自动充值；启用前应设置触发余额、充值金额和页面提供的月度限制。ChatGPT Credits 不是 OpenAI Platform API Credits，不能转让、转售或兑换现金。
...
OpenAI 当前帮助文档说明：当相关功能对套餐可用时，Codex、ChatGPT Work、ChatGPT for Excel 和 Workspace Agents 会从同一个 agentic usage 与 credit pool 扣减。因此，即使没有继续使用 Codex CLI，同一账号里的其他受支持 agentic 功能也可能让剩余用量下降；具体可用功能仍以账号页面为准。

Codex 也不是固定的“每发一条扣一次”。当前 rate card 对绝大多数账号按输入、缓存输入和输出 Token 映射 Credits；消耗还会随模型、代码库和任务复杂度、上下文大小以及本地或云端运行而变化。少数尚未迁移的 Enterprise 账号仍需查看 legacy rate card。
...
- **ChatGPT Credits**：套餐包含用量先消耗，达到限制后，符合条件的账号才从余额扣减；它不是 OpenAI Platform API 余额。
- **Banked rate-limit reset**：部分 Plus / Pro 推荐活动会提供可保存的 Codex 限额重置。它不是 Credits，也不形成现金、API 或可转让余额；除非活动另有说明，通常要在入账后 30 天内使用。
- **临时额外用量或测试变化**：活动可能单独提供一段临时用量；个别账号一段时间内看不到原限制，也不代表 OpenAI 永久取消所有 5 小时或周限制。
...
官方参考：[Codex Pricing](https://learn.chatgpt.com/docs/pricing)、[Using Codex with your ChatGPT plan](https://help.openai.com/en/articles/11369540-using-codex-with-your-chatgpt-plan)、[Using Credits for Flexible Usage in ChatGPT](https://help.openai.com/en/articles/12642688)、[Codex Referral Promotions](https://help.openai.com/en/articles/20001271-codex-referral-promotions)、[Codex rate card](https://help.openai.com/en/articles/20001106-codex-rate-card)。
...
付款后不知道去哪里查订单、余额或订阅状态时，使用独立的 [ChatGPT 充值记录、订单号与订阅状态安全核对清单](guides/chatgpt-order-history-subscription-status.md)，并避免把 Session 粘贴到第三方查询工具。
...
### ChatGPT Plus 多少钱一个月，20 美元换算人民币为什么不固定？
...
独立的人民币计算与渠道检查表见：[ChatGPT Plus 多少钱一个月：人民币价格、税费与多币种](guides/chatgpt-plus-price-rmb-tax.md)。完整价格表与渠道对照见：[ChatGPT Plus 人民币价格、税费与多币种说明](https://hi-codex.com/guides/chatgpt-recharge-price/?utm_source=github&utm_medium=readme&utm_campaign=price_currency_tax)。
...
- [MCP Server 添加、OAuth、Token 与工具审批](guides/codex-mcp-configuration.md)
